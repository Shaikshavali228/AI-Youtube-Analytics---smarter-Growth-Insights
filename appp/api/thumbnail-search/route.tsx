...
//         {
//           "type": "text",
//           "text": `Describe this thumbnail in short keywords suitable for searching similar YouTube videos. Give me tags with comma separated. Do not give any comment text, Maximum 5 tags.
//            Make sure after searching that tags will get similer youtube thumbnails`
//         },
//         {
//           "type": "image_url",
//           "image_url": {
...
export async function GET(req: NextRequest) {
  ...
  if (thumbnailUrl) {
    ...
    if (Array.isArray(resultContent)) {
      resultContent = resultContent.map((c: any) => c.text).join(" ");
    }
    query = resultContent;
  }
  ...
  const FinalResult = videoResp.data.items.map((item: any) => ({
    id: item.id,
    title: item.snippet.title,
